...
class ToolRunner:
    ...
    def run(
        ...
        # ── Dry run ──
        if self.dry_run:
            cmd_str = self._redact_command_for_logging(full_cmd)
    ...
    def _redact_command_for_logging(cmd: List[str]) -> str:
        """
        Redact likely secrets from a command before writing to logs.
        """
        rendered = " ".join(cmd)
        ...
        rendered = re.sub(r"(://[^\s:@]+):([^\s@]+)@", r"\1:***@", rendered)
        rendered = re.sub(r"(\s(?:-p|--password)\s+)(\S+)", r"\1***", rendered)
